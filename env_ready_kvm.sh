...
#if have internal yum repo:
#set_local_repo.sh
yum install -y wget net-tools sysstat vim curl git chrony \
...
grep net.ifnames=0 /boot/grub2/grub.conf
[ $? = 0 ] && echo "Already set grub, skip" || sed -i.ori 's/rhgb/net.ifnames=0 biosdevnam=0 rhgb/g' /boot/grup2/grub.conf
...
#echo kvmbase2 > /etc/hostname;hostname -f kvmbase2
...
KERNEL-DEVEL=${KURL}kernel-ml-devel-4.18.12-1.el7.elrepo.x86_64.rpm
